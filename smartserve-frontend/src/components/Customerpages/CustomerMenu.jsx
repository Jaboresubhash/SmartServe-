
import React, { useEffect, useState } from "react";
import API from "../../api/api";
import {
  Button,
  Card,
  CardContent,
  CardMedia,
  Typography,
  Grid,
  TextField,
  Box,
  Divider,
  Stack,
} from "@mui/material";

const CustomerMenu = () => {
  const [menu, setMenu] = useState([]);
  const [filteredMenu, setFilteredMenu] = useState([]);
  const [cart, setCart] = useState([]);
  const [tableNo, setTableNo] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");

  // 🔹 Fetch menu items
  useEffect(() => {
    API.get("/menu")
      .then((res) => {
        setMenu(res.data);
        setFilteredMenu(res.data);
      })
      .catch((err) => console.log(err));
  }, []);

  // ---- CART HELPERS (place these above the return) ----
  const getCartItem = (id) => {
    return cart.find((c) => c.id === id);
  };

  // Add to cart (if exists -> increase, else add with qty 1)
  const addToCart = (item) => {
    setCart((prev) => {
      const existing = prev.find((c) => c.id === item.id);
      if (existing) {
        return prev.map((c) => (c.id === item.id ? { ...c, qty: c.qty + 1 } : c));
      } else {
        return [...prev, { ...item, qty: 1 }];
      }
    });
  };

  const increaseQty = (itemId) => {
    setCart((prev) => prev.map((c) => (c.id === itemId ? { ...c, qty: c.qty + 1 } : c)));
  };

  const decreaseQty = (itemId) => {
    setCart((prev) =>
      prev
        .map((c) => (c.id === itemId ? { ...c, qty: c.qty - 1 } : c))
        .filter((c) => c.qty > 0)
    );
  };

  // 🔹 Helper: apply both category + search filter
  const applyFilters = (category, search) => {
    let result = [...menu];

    // category filter
    if (category !== "All") {
      result = result.filter((item) => item.category === category);
    }

    // search filter (by name)
    if (search.trim() !== "") {
      const lower = search.toLowerCase();
      result = result.filter((item) => item.name.toLowerCase().includes(lower));
    }

    setFilteredMenu(result);
  };

  // 🔹 Filter menu based on category
  const handleCategoryFilter = (category) => {
    setSelectedCategory(category);
    applyFilters(category, searchTerm);
  };

  // 🔹 Handle search text change
  const handleSearchChange = (e) => {
    const value = e.target.value;
    setSearchTerm(value);
    applyFilters(selectedCategory, value);
  };

  // 🔹 Place order
  const placeOrder = () => {
    if (!tableNo) {
      alert("Enter Table Number");
      return;
    }
    if (cart.length === 0) {
      alert("Cart is empty");
      return;
    }

    API.post("/orders", { table_no: tableNo, items: cart })
      .then(() => {
        alert("Order Placed!");
        setCart([]);
        setTableNo("");
      })
      .catch((err) => {
        console.log(err);
        alert("Failed to place order");
      });
  };

  // calculate cart totals (optional)
  const totalItems = cart.reduce((acc, cur) => acc + (cur.qty || 0), 0);
  const totalAmount = cart.reduce((acc, cur) => acc + (cur.qty || 0) * (cur.price || 0), 0);

  return (
    <Box sx={{ display: "flex", backgroundColor: "#f8f9fa" }}>
      {/* ================= LEFT SIDEBAR ================= */}
      <Box
        sx={{
          width: 250,
          height: "83vh",
          bgcolor: "#2c3e50",
          color: "white",
          p: 3,
          display: "flex",
          flexDirection: "column",
          justifyContent: "space-between",
          boxShadow: 3,
        }}
      >
        <Box>
          <Typography variant="h5" sx={{ fontWeight: "bold", mb: 3, textAlign: "center" }}>
            🍴 Categories
          </Typography>

          <Stack spacing={2}>
            {["All", "Tiffin", "Main Course", "Dessert", "Beverages"].map((cat) => (
              <Button
                key={cat}
                variant={selectedCategory === cat ? "contained" : "outlined"}
                sx={{
                  color: selectedCategory === cat ? "white" : "#fff",
                  backgroundColor: selectedCategory === cat ? "#00796b" : "transparent",
                  "&:hover": {
                    backgroundColor:
                      selectedCategory === cat ? "#004d40" : "rgba(255,255,255,0.1)",
                  },
                  textTransform: "capitalize",
                  fontWeight: "bold",
                }}
                onClick={() => handleCategoryFilter(cat)}
                fullWidth
              >
                {cat}
              </Button>
            ))}
          </Stack>
        </Box>

        <Divider sx={{ bgcolor: "rgba(255,255,255,0.3)", my: 3 }} />

        <Box>
          <Typography variant="body2" sx={{ textAlign: "center", color: "gray" }}>
            © {new Date().getFullYear()} SmartServe
          </Typography>

          {/* small cart summary in sidebar */}
          <Box sx={{ mt: 2, textAlign: "center" }}>
            <Typography variant="body2" sx={{ color: "white" }}>
              Items: {totalItems}
            </Typography>
            <Typography variant="body2" sx={{ color: "white", fontWeight: "bold" }}>
              Total: ₹{totalAmount.toFixed(2)}
            </Typography>
          </Box>
        </Box>
      </Box>

      {/* ================= RIGHT SIDE CONTENT ================= */}
      <Box
        sx={{
          flexGrow: 1,
          p: 3,
          overflowY: "scroll",
          height: "83vh",
          width: "71vw",
          "&::-webkit-scrollbar": { width: 0 },
          scrollbarWidth: "none", // Firefox
        }}
      >
        <Typography variant="h4" align="center" gutterBottom
          sx={{ fontWeight: "bold", color: "#333", mb: 4, letterSpacing: 1 }}>
          Explore Our Delicious Menu 🍽️
        </Typography>

        {/* 🔹 Table Number + Search Row */}
        <Box display="flex" justifyContent="center" gap={2} mb={4} flexWrap="wrap">
          <TextField
            label="Enter Table Number"
            value={tableNo}
            onChange={(e) => setTableNo(e.target.value)}
            sx={{ width: 250, backgroundColor: "white", borderRadius: 2 }}
          />

          {/* 🔎 Search Bar */}
          <TextField
            label="Search Food Name"
            value={searchTerm}
            onChange={handleSearchChange}
            sx={{ width: 250, backgroundColor: "white", borderRadius: 2 }}
          />
        </Box>

        {/* 🔹 Menu Grid */}
        <Grid container spacing={3} justifyContent="center">
          {filteredMenu.length > 0 ? (
            filteredMenu.map((item) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={item.id}>
                <Card
                  sx={{
                    height: 380,
                    width: "20vw",
                    display: "flex",
                    flexDirection: "column",
                    justifyContent: "space-between",
                    borderRadius: 3,
                    boxShadow: "0 6px 10px rgba(0,0,0,0.1)",
                    transition: "transform 0.3s, box-shadow 0.3s",
                    "&:hover": {
                      transform: "translateY(-8px)",
                      boxShadow: "0 10px 20px rgba(0,0,0,0.15)",
                    },
                  }}
                >
                  <CardMedia
                    component="img"
                    height="180"
                    image={
                      item.img
                        ? `http://localhost:5000/uploads/${item.img}`
                        : "https://via.placeholder.com/300x180?text=No+Image"
                    }
                    alt={item.name}
                  />
                  <CardContent sx={{ textAlign: "center" }}>
                    <Typography variant="h6" sx={{ fontWeight: 600, textTransform: "capitalize" }}>
                      {item.name}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                      {item.category}
                    </Typography>
                    <Divider sx={{ mb: 1 }} />
                    <Typography variant="body1" sx={{ color: "#00796b", fontWeight: "bold", mb: 2 }}>
                      ₹{item.price}
                    </Typography>

                    {/* Cart control: if item in cart show - qty +, else show Add to Cart */}
                    {getCartItem(item.id) ? (
                      <Box
                        sx={{
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "space-between",
                          gap: 1,
                          borderRadius: 2,
                          border: "1px solid rgba(0,0,0,0.08)",
                          padding: "6px",
                        }}
                      >
                        <Button
                          variant="outlined"
                          size="small"
                          onClick={() => decreaseQty(item.id)}
                          sx={{ minWidth: 36, height: 36, padding: 0 }}
                        >
                          −
                        </Button>

                        <Typography sx={{ fontWeight: 700, minWidth: 24, textAlign: "center" }}>
                          {getCartItem(item.id).qty}
                        </Typography>

                        <Button
                          variant="outlined"
                          size="small"
                          onClick={() => increaseQty(item.id)}
                          sx={{ minWidth: 36, height: 36, padding: 0 }}
                        >
                          +
                        </Button>
                      </Box>
                    ) : (
                      <Button
                        variant="contained"
                        sx={{
                          backgroundColor: "#ff7043",
                          "&:hover": { backgroundColor: "#e64a19" },
                        }}
                        onClick={() => addToCart(item)}
                        fullWidth
                      >
                        Add to Cart
                      </Button>
                    )}
                  </CardContent>
                </Card>
              </Grid>
            ))
          ) : (
            <Typography variant="h6" color="text.secondary">
              No items available for this search.
            </Typography>
          )}
        </Grid>

        {/* 🔹 Place Order Button */}
        {cart.length > 0 && (
          <Box textAlign="center" mt={5}>
            <Button
              variant="contained"
              color="primary"
              sx={{
                px: 5,
                py: 1.5,
                borderRadius: 3,
                fontWeight: "bold",
                backgroundColor: "#00796b",
                "&:hover": { backgroundColor: "#004d40" },
              }}
              onClick={placeOrder}
            >
              Place Order ({totalItems} items) — ₹{totalAmount.toFixed(2)}
            </Button>
          </Box>
        )}
      </Box>
    </Box>
  );
};

export default CustomerMenu;

